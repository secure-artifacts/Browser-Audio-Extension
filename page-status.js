...
const microphoneErrors = {
  ...
  OverconstrainedError: "设备无法满足音频要求。可关闭插件、刷新网页后重试。",
...
function summarizeFrames(results, enabled, version) {
  ...
  if (count("processing")) return { code: "processing", tone: "warn", text: "音频处理仍有开启项", hint: "尚未完全生效。请刷新、重新通话后再检测。" };
  if (count("live")) {
    ...
    if (count("muted")) return { code: "muted", tone: "warn", text: "处理已关闭 · 有音轨暂停", hint: "检测到音轨静音或暂时无数据，请检查网站静音按钮和设备。" };
    return { code: "verified", tone: "good", text: "检测到音频处理已关闭", hint: "已检查的麦克风音轨符合设置；对方能否听见仍以通话为准。" };
  }
